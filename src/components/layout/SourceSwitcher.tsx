...
import { Select, SelectContent, SelectItem, SelectTrigger, SelectSeparator } from '../ui/select';
    ...
    <Select value={currentSourceId?.toString() || ''} onValueChange={handleSelect}>
      ...
      <SelectContent className="border-white/10 bg-[#0B0C0E] text-white">
        <div className="max-h-[300px] overflow-y-auto">
          {sources.length === 0 && (
            <div className="px-2 py-4 text-center text-sm text-white/50">No sources found</div>
          )}
          {sources.map((source) => (
            <SelectItem
              key={source.id}
              value={source.id.toString()}
              className="cursor-pointer py-2 focus:bg-white/10"
            >
        ...
        <SelectItem value="create" className="cursor-pointer focus:bg-white/10">
          <div className="flex items-center gap-2 text-white/60">
            <Plus className="h-4 w-4" />
            <span>Add Source</span>
          </div>
        </SelectItem>
        <SelectItem value="manage" className="cursor-pointer focus:bg-white/10">
          <div className="flex items-center gap-2 text-white/60">
