...
export class EventPayload {
  ...
  get recipients(): string[] {
    switch (this.eventType) {
      case 'Bounce':
        return this.bounce?.bouncedRecipients
          ?.map((recipient) => recipient?.emailAddress)
          .filter((value): value is string => typeof value === 'string') ?? []
      case 'Complaint':
        return this.complaint?.complainedRecipients
          ?.map((recipient) => recipient?.emailAddress)
          .filter((value): value is string => typeof value === 'string') ?? []
      case 'Delivery':
      ...
      case 'DeliveryDelay':
        return this.deliveryDelay?.delayedRecipients
          ?.map((recipient) => recipient?.emailAddress)
          .filter((value): value is string => typeof value === 'string') ?? []
      default:
        return (Array.isArray(this.mail.destination)
          ? this.mail.destination
          : []
        ).filter((value): value is string => typeof value === 'string')
    }
  }
  ...
  private get delivery() {
    return this.raw.delivery as
      | { timestamp?: unknown; recipients?: unknown[] }
      | undefined
  }
  ...
  private get subscription() {
    return this.raw.subscription as
      | { timestamp?: unknown }
      | undefined
  }
}
